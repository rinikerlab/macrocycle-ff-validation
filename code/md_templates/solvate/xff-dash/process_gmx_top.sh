...
elif [ "$1" == "remove_newlines" ]; then
    if [ -z "$2" ]; then
        echo "usage: $0 remove_newlines FLAG [TOP]"
        exit 1
    fi
    flag="$2"
    awk -v flag=$flag "$AWK_REMOVE_NEWLINES" $3
else
    echo "commands: remove prepend append remove_newlines; For help: $0 COMMAND"
fi
